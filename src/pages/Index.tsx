...
const Index = () => {
  ...
  const getSeverityInfo = (content: string) => {
    ...
    if (lowerContent.includes('severe')) {
      return {
        badge: <Badge className="bg-red-500 text-white text-xs px-2 py-1">Severe</Badge>,
        urgency: 'high',
        color: 'red'
      };
    }
    if (lowerContent.includes('moderate')) {
      return {
        badge: <Badge className="bg-orange-500 text-white text-xs px-2 py-1">Moderate</Badge>,
        urgency: 'medium',
        color: 'orange'
      };
    }
    if (lowerContent.includes('mild')) {
      return {
        badge: <Badge className="bg-yellow-600 text-white text-xs px-2 py-1">Mild</Badge>,
        urgency: 'low',
        color: 'yellow'
      };
    }
    if (lowerContent.includes('healthy')) {
      return {
        badge: <Badge className="bg-green-500 text-white text-xs px-2 py-1">Healthy</Badge>,
        urgency: 'none',
        color: 'green'
      };
    }
  ...
  const renderStructuredContent = (content: string, isRecommendation = false) => {
    ...
    return <div className="space-y-3">
        ...
        if (line.trim().startsWith('-')) {
          ...
          return <div key={index} className="flex items-start justify-between group py-2 px-3 rounded-lg bg-gray-50/50 hover:bg-gray-100/50 transition-colors">
                <div className="flex items-start space-x-3 flex-1">
                  <div className={`w-2 h-2 rounded-full mt-2 flex-shrink-0 ${isRecommendation ? 'bg-blue-500' : 'bg-gray-400'}`}></div>
                  <span className="text-gray-700 text-sm leading-relaxed flex-1">
                    {text}
                  </span>
                </div>
                {isRecommendation && <Button size="sm" variant="ghost" className="opacity-0 group-hover:opacity-100 transition-opacity h-6 w-6 p-0" onClick={() => copyToClipboard(text)}>
  ...
  return <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white px-4 py-4 flex items-center shadow-sm">
        <img src="/lovable-uploads/0b150c40-a73b-479d-9cce-d0ffa6576f85.png" alt="Kisan-G Logo" className="h-8 mr-3" />
        <h1 className="text-xl font-medium text-lime-500"></h1>
      </div>
      ...
      <div className="px-4 py-6 space-y-6">
        ...
        {analysis && !isAnalyzing && <div className="space-y-6">
          ...
          return <div className="space-y-8">
                  {/* Plant Identification */}
                  {groups.identification.map((section, index) => <Card key={index} className="bg-white rounded-xl shadow-sm border-0 ring-2 ring-green-100 bg-green-50/30">
              ...
              return <Card key={index} className={`bg-white rounded-xl shadow-sm border-0 ring-2 ring-${severityInfo.color}-100 bg-${severityInfo.color}-50/30`}>
                        <CardContent className="p-5">
                  ...
                  {groups.analysis.length > 0 && <div className="space-y-4">
                      <h4 className="text-lg font-semibold text-gray-900 px-2">Analysis Details</h4>
                      <div className="grid gap-4 md:grid-cols-2">
                        {groups.analysis.map((section, index) => <Card key={index} className="bg-white rounded-xl shadow-sm border-0">
                            <CardContent className="p-5">
                              <div className="flex items-center space-x-3 mb-4">
                  ...
                  {/* Treatment Recommendations */}
                  {groups.treatment.map((section, index) => <Card key={index} className="bg-white rounded-xl shadow-sm border-0 ring-2 ring-blue-100 bg-blue-50/30">
                      <CardContent className="p-5">
                        <div className="flex items-center justify-between mb-4">
                          <div className="flex items-center space-x-3">
                          ...
                          <Button size="sm" variant="outline" onClick={() => copyToClipboard(section.content)} className="text-blue-600 border-blue-200 hover:bg-blue-50">
                  ...
                  <div className="grid gap-4 md:grid-cols-2">
                    {[...groups.prevention, ...groups.prognosis].map((section, index) => <Card key={index} className="bg-white rounded-xl shadow-sm border-0">
                        <CardContent className="p-5">
                          <div className="flex items-center space-x-3 mb-4">
                            <div className="w-10 h-10 bg-gray-100 rounded-full flex items-center justify-center">
                              {getIcon(section.title)}
